 ...
 *
 * We read all SCSS files in `src/00-config/settings/`, extract top-level variable
 * declarations (lines starting with `$`), then verify each exists in either the
 * generated output or the compat layer.
 *
 ...
 * Matches lines that start with `$variable-name:` (with optional leading whitespace).
 * Does NOT match variables inside maps or used as values.
 */
const VARIABLE_DECLARATION_RE = /^\$([a-zA-Z_][\w-]*)\s*:/;

/**
 * Extract all top-level SCSS variable names from a file.
 * Only matches lines starting with `$` (after optional whitespace),
 * excluding lines inside map bodies (indented with spaces/tabs before `$`).
 */
function extractTopLevelVariables(filePath) {
  ...
  for (const line of lines) {
    // Only match lines where $ is at the very start (top-level declarations)
