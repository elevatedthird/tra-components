...
(function ($, Drupal, once) {
  Drupal.behaviors.sqTabSlider = {
    ...
    init(glide, element) {
      const tabSlider = element.closest('.molecule--sq-tabs-item--tabs-content');
    ...
    attach(context) {
      const tabSliders = once('sequoia-tab-slider', context.querySelectorAll('.paragraph--type--tab-teaser-slider'));
      tabSliders.forEach(slider => {
        const blockWrapper = slider.closest('.block--tab-slider');
        if (blockWrapper) {
          const tabList = blockWrapper.querySelector('.tab-list');
          if(!tabList) return;
          const ul = tabList.querySelector(".tabs__tab-list");
          if(!ul) return;
          const tabs=ul.querySelectorAll("li[role='presentation']");
          if (tabs.length === 1){
